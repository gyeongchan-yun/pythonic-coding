...
def nonblocking_lock(lock):
    ...
    while grab < 3:
        ...
        success = lock.acquire(False)  # acquire(Block, timeout)
...
def main():
    ...
    blocking = threading.Thread(name="blocking", target=blocking_lock, args=(lock,))
    blocking.setDaemon(True)
    blocking.start()

    non_blocking = threading.Thread(name="nonblocking", target=nonblocking_lock, args=(lock,))
    non_blocking.start()
